...
HOME_FAQ = [
    ("인천 전지역 어디까지 방문 가능한가요?",
     "강화군·옹진군과 중구·동구·미추홀구·연수구·남동구·부평구·계양구·서구 등 인천 전역을 안내드립니다. 강화·옹진의 도서 지역은 선편 일정에 따라 사전 협의가 필요하며, 정확한 방문 가능 여부는 위치와 예약 시간에 따라 상담 시 확인해 드립니다."),
    ("지하철역 근처에서도 예약할 수 있나요?",
     "인천1·2호선, 1호선(경인선) 인천권, 7호선 인천권, 수인분당선 인천권, 공항철도 인천권 주요 역세권으로 방문 안내가 가능합니다. 부평역·주안역·인천시청역·송도역 등 대표 역 인근은 지하철역별 안내에서 확인하실 수 있습니다."),
    ("당일 예약도 가능한가요?",
     "가능합니다. 다만 시간대와 위치, 배정 상황에 따라 방문 가능 시간이 달라질 수 있어 상담 시 확인해 드립니다."),
    ("테마(스웨디시·타이 등)는 어떻게 선택하나요?",
     "스웨디시·아로마테라피·타이마사지·홈케어 등 테마는 테마별 안내에서 설명을 확인하신 뒤, 예약 시 컨디션과 목적을 말씀해 주시면 적합한 관리를 안내드립니다."),
    ("예약 전 무엇을 준비하면 되나요?",
     "방문 장소의 정확한 주소와 출입 방법, 연락 가능한 번호, 희망 시간과 코스를 준비해 주시면 빠르게 진행됩니다. 자세한 내용은 이용 전 확인사항에서 확인하실 수 있습니다."),
    ("이 서비스는 의료 행위인가요?",
     "아닙니다. 본 서비스는 의료 행위가 아닌 이완·휴식 목적의 방문 건강관리 서비스이며, 만 19세 이상 성인을 대상으로 합니다."),
]
...
def build_home():
    services = "".join(
        ...
        for c in COURSES[:4])
    # 지역 카드 — 카드명에 '출장마사지' 반복 금지(구/군명 + 요약)
    regions = "".join(
    ...
    themes = "".join(
        ...
        for t in THEMES[:8])
    steps = [
        ("위치 확인", "지역 또는 가까운 지하철역 인근 위치를 알려주세요."),
        ("시간·코스 확인", "희망 시간과 코스, 인원을 함께 확인합니다."),
        ("방문 가능 여부 안내", "배정 상황을 확인해 방문 가능 시간을 안내드립니다."),
        ("예약 확정", "확정 후 관리사가 약속된 시간에 방문합니다."),
    ]
    steps_html = "".join(
    ...
    check_notes = [
        ("이용 전 확인사항", ["정확한 주소와 공동현관 출입 방법, 주차 가능 여부를 미리 알려주시면 도착이 빨라집니다.",
                         "조용히 쉴 수 있는 공간과 예약자 본인이 연락 가능한 번호를 준비해 주세요."]),
        ("위생 및 안전 안내", ["용품 위생 관리와 개인정보 보호를 기본 기준으로 운영합니다.",
                         "본 서비스는 의료 행위가 아닌 건강관리 서비스이며, 만 19세 이상을 대상으로 불법·퇴폐 행위는 일절 제공하지 않습니다."]),
    ]
...
<section class="hero"><div class="hero-inner">
  <div class="hero-copy">
    ...
    <p class="lead">강화·옹진부터 부평·주안·송도·청라까지 — 인천 전지역 출장마사지·홈타이 예약을 연중무휴로 안내드립니다.</p>
    <div class="actions">
...
<section class="block"><div class="wrap">
  ...
  <p class="sec-lead">고객이 계신 장소로 찾아가는 방문형 건강관리 서비스입니다. 예약 시 위치·시간·코스를 확인한 뒤 건전한 방문 관리 기준에 따라 진행합니다.</p>
...
<section class="block" id="region"><div class="wrap">
  ...
  <p class="sec-lead">인천 2군·8구를 기준으로 안내합니다. 2026년 7월 1일 행정체제 개편(제물포구·영종구·검단구 출범)이 예정되어 있어, 개편 이후 메뉴와 주소 안내를 함께 업데이트합니다.</p>
...
<section class="block" id="stations"><div class="wrap">
  ...
  <p class="sec-lead">인천1·2호선, 1호선(경인선) 인천권, 7호선 인천권, 수인분당선·공항철도 인천권 역세권으로 방문 안내가 가능합니다.</p>
...
<section class="block" id="theme"><div class="wrap">
  ...
  <p class="sec-lead">스웨디시·아로마테라피·타이마사지·호텔식마사지·스포츠·경락 등 목적에 맞는 테마를 안내합니다.</p>
...
def build_gu_pages():
    for g in GU:
        ...
        for d in g["dongs"]:
            dong_lines.append(
                f'<a href="/incheon/{g["slug"]}/{d["slug"]}/">{d["name"]}</a>은(는) {d["character"]}으로, '
                f'{d["landmarks"]}를 중심으로 {arr_text(d)} 방문합니다.')
        sections = [
            (f"{g['name']} 출장마사지·홈타이 안내", [
                f"{g['name']}은(는) {g['summary']}",
                f"{g['name']}에서는 자택·오피스텔·숙소로 방문하는 출장마사지·홈타이 예약을 안내드립니다. 매장을 찾지 않고도 익숙한 공간에서 피로 회복과 이완을 받을 수 있으며, 아래 대표 동을 눌러 동별 생활권과 방문 안내를 확인하실 수 있습니다.",
                f"행정동이 1동·2동처럼 여러 개로 나뉜 곳은 대표 동 하나로 통합해 안내하므로, 같은 이름의 세부 행정동도 대표 동 페이지에서 함께 확인하시면 됩니다. 예약 시 가까운 지하철역이나 큰 건물을 함께 알려주시면 위치 파악이 빨라집니다."]),
            (f"{g['name']} 방문 가능 동", [
                f"{g['name']}은 " + "·".join(d["name"] for d in g["dongs"]) + " 일대를 포함합니다. 행정동이 여러 개로 나뉜 지역은 대표 동으로 통합해 안내합니다.",
            ...
            (f"{g['name']} 방문 예약 팁", [
                f"{g['name']}은(는) {g['summary']} 이런 지역 특성에 맞춰, 예약 시 가까운 지하철역이나 큰 건물·교차로 등 기준점을 함께 알려주시면 위치 파악과 도착이 빨라집니다.",
                "저녁 시간대와 주말에는 문의가 몰릴 수 있어, 원하는 시간이 정해져 있다면 미리 예약하시길 권합니다.",
...
def _dong_zone_blocks(g, d):
    tokens = [t.strip() for t in d["landmarks"].replace("·", ", ").split(",") if t.strip()]
    sd = d["slug"]
    blocks = []
    if d.get("arrival"):
    ...
    blocks += [("h3", "주거지·숙소 방문 안내"),
               _pick(sd, [
                   ...
                   f"{d['name']} 방문은 {tokens[0].replace(' 일대','')} 일대 자택·숙소를 기준으로 이뤄집니다. 정확한 주소와 동·호수, 출입 방법을 미리 남겨주시면 도착이 빨라집니다.",
                   f"{tokens[0].replace(' 일대','')}를 비롯한 {d['name']} 생활권의 자택·오피스텔·숙소로 방문하며, 출입 방법과 연락 가능한 번호를 함께 알려주시면 좋습니다."], "n1"),
               ("h3", "방문 동선·주차 안내"),
               f"{d['name']}은(는) {d['character']}으로, {_pick(sd, ['방문 차량 주차 가능 여부와 위치를 예약 시 함께 확인하면 도착 후 진행이 빠릅니다.', '주차 가능 여부를 미리 알려주시면 도착 후 대기 없이 진행할 수 있습니다.', '주차 공간이 마땅치 않은 곳은 인근 주차 위치를 함께 안내드립니다.'], 'n2')} 건물 출입이 까다로운 곳은 경비실·공동현관 안내 방법을 미리 알려주시면 좋습니다."]
    return blocks
